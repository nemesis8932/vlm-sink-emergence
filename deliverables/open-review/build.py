...
# `--unslop` builds from sections-unslop/, the AI-tell-stripped edit of the body. The abstract
# and the conclusion are byte-identical copies of sections/; §1-§5 and the appendix are rewritten.
UNSLOP = "--unslop" in sys.argv
SRC = "sections-unslop" if UNSLOP else "sections"
SECTIONS = sorted((HERE / SRC).glob("[0-9][0-9]-*.md"))
...
h1.title { font-size: 16.5pt; text-align: center; line-height: 1.25;
           ...
           border-top: 4.5px solid #111; border-bottom: 1.4px solid #111; }
...
"""
    ...
    draft = f"draft-{SLUG.removeprefix('paper-')}.md"
