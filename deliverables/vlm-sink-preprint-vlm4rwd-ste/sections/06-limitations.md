...
other positions (per-position table in Appendix E). The pos0-anchored textinit magnitudes at those
...
initialization (§3). Gating and initial output scaling are therefore confounded in *g1gate*. We
...
a metric-labeling error in an earlier internal consolidation (Appendix H). *baseline* and *sigmoid* have two seeds, *g1gate* and *textinit* three, RF a **single seed**. RF
