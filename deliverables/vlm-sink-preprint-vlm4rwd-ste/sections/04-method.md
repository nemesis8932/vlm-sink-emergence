...
There are two seeds each for baseline and sigmoid, and three for g1gate and textinit.
...
batch. Excluding query 0 drops the trivial self-attention term that [6] includes, so our
estimate is slightly stricter. Sigmoid weights are row-normalized for this diagnostic,
following [6]. The model's
