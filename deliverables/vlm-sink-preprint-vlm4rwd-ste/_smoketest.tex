...
\begin{figure}[t]
  ...
  \caption{\textbf{Figure A5: Birth-maps.} The step at which each (layer, query-head) pair first crosses the concentration threshold (attn$\rightarrow$pos0 $>$ 0.3), seed 0, for the three arms in which any head crosses. No \textit{baseline} and no \textit{RF} head ever crosses. 1\% of \textit{g1gate} heads do, against 89\% for \textit{sigmoid} and 87\% for \textit{textinit}. This is the head-level view behind Fig. A4.}
  \label{fig:figA5}
\end{figure}
