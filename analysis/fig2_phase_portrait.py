...
def draw(ax, xs, ys_raw, color, label):
    ...
    ax.scatter(xs[idx], ys[idx], s=np.linspace(9, 26, len(idx)), color=color,
               alpha=0.45, zorder=5, edgecolors='none')
    # init = open circle (raw values), final = diamond (raw values)
    ax.scatter([xs[0]], [ys_raw[0]], s=52, facecolors='white', edgecolors=color,
               lw=1.6, zorder=7)
    ax.scatter([xs[-1]], [ys_raw[-1]], s=110, color=color, marker='D',
               edgecolors='white', lw=1.3, zorder=8)
...
def main():
    ...
    for ax in (axL, axR):
        ...
        fc.style_ax(ax)
        ax.set_xlabel('attention concentration  (max attn→pos0)', fontsize=9.5)

    axL.set_ylabel('value-norm ratio   ‖v‖$_{pos0}$ / ‖v‖$_{rest}$', fontsize=9.5)
    axL.set_title('Concentration vs value-norm', fontsize=10.5, weight='bold', pad=8)
    axL.text(0.02, 0.965, '↑ value amplified', transform=axL.transAxes, fontsize=7.5,
             color='#777777', style='italic')
    axL.text(0.02, 0.035, '↓ value drained', transform=axL.transAxes, fontsize=7.5,
             color='#777777', style='italic')

    axR.set_yscale('log')
    axR.set_ylabel('massive-activation ratio   ‖h‖$_{pos0}$ / ‖h‖$_{rest}$   (log)',
                   fontsize=9.5)
    axR.set_title('Concentration vs massive-activation', fontsize=10.5, weight='bold', pad=8)
    ...
    fig.legend(handles=handles, loc='lower center', ncol=7, fontsize=7.8, frameon=False,
               bbox_to_anchor=(0.5, -0.035), handlelength=1.7, columnspacing=1.4)
    fig.suptitle('Decoupling phase-portrait: each lever drives the three sink signatures '
                 'into a different corner', fontsize=11.5, weight='bold', y=1.0)
